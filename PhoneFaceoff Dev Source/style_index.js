...
$(document).ready( function() {
	...
	// Gets the phone name from search page and puts in first input field
	$(document).ready( function() {
		var search = localStorage.getItem('search');
		setTimeout(function() {
			if (search) {
				var elem = "#phone-input-field-1";
				$(elem).val(search);
				table.update( function() {table.send(table.args)}, table.args, $(elem).attr('id'), viewport.check() );
				image.update( function() {image.send(image.args)}, image.args, $(elem).attr('id'), viewport.check() );
				suggest.update( function() {suggest.send(suggest.args)}, suggest.args, $(elem).attr('id'), viewport.check() );
				rating.update( function() {rating.send(rating.args)}, rating.args, $(elem).attr('id'), viewport.check() );
